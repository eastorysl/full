...
import { FiMapPin, FiNavigation, FiSun, FiClock, FiDollarSign, FiAward, FiCrosshair, FiTrendingUp, FiArrowRight } from 'react-icons/fi'
...
function PopularCard({ dest, i, distance }) {
...
export default function PopularNearYou() {
  ...
  const popularPlaces = useMemo(() => {
    ...
    let pool = [...destinations].sort((a, b) => {
    ...
    if (activeCategory !== 'All') {
      ...
      const filtered = pool.filter(d => d.category === cat)
  ...
  return (
    <section className="section-padding relative overflow-hidden bg-gradient-to-b from-slate-50/50 via-white to-slate-50/30">
      ...
      <div className="container-custom relative z-10">
        <SectionTitle
          ...
          description="Top-rated destinations popular with travelers, sorted by tier and proximity."
        />
        ...
        {popularPlaces.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6 max-w-5xl mx-auto mb-10">
            <AnimatePresence mode="popLayout">
              {popularPlaces.map((dest, i) => (
                <PopularCard key={dest.id} dest={dest} i={i} distance={dest.distance} />
              ))}
            </AnimatePresence>
          </div>
        )}

        {popularPlaces.length === 0 && locating && (
        ...
        {popularPlaces.length === 0 && locateError && (
          <div className="text-center py-10">
